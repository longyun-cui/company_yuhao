...
class WLStaffMotorcadeRepository {
     ...
     */
    // 【车队】返回-列表-数据
    public function o1__motorcade__list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;


        $query = WL_Common_Motorcade::select(['id','item_status','name','motorcade_category','motorcade_type','leader_id','remark','creator_id','created_at','updated_at','deleted_at'])
            ->withTrashed()
            ->with([
        ...
        {
            $query->where('superior_motorcade_id',$me->motorcade_district_id);
            $query->whereNotIn('superior_motorcade_id', [-1,0]);
        }
...
//        dd($list->toArray());
        ...
        foreach($list as $k => $v)
        {
            if($v->motorcade_type == 11)
            {
                $v->district_id = $v->id;
            }
            else if($v->motorcade_type == 21)
            {
                $v->district_id = $v->superior_motorcade_id;
            }
...
//        dd($list->toArray());
    ...
    // 【车队】获取 GET
    public function o1__motorcade__item_get($post_data)
    {
        $messages = [
        ...
        $item = WL_Common_Motorcade::withTrashed()
            ->with([
    ...
    // 【车队】保存 SAVE
    public function o1__motorcade__item_save($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'motorcade_category.required' => '请选择车队类型！',
            'name.required' => '请输入车队名称！',
//            'name.unique' => '该车队号已存在！',
        ];
        $v = Validator::make($post_data, [
            'operate' => 'required',
            'motorcade_category' => 'required',
            'name' => 'required',
//            'name' => 'required|unique:WL_Common_Motorcade,name',
        ], $messages);
        if ($v->fails())
        {
            $messages = $v->errors();
        ...
        {
            $is_exist = WL_Common_Motorcade::select('id')->where('name',$post_data["name"])->count();
            if($is_exist) return response_error([],"该【车队】已存在，请勿重复添加！");

            $mine = new WL_Common_Motorcade;
            $post_data["active"] = 1;
            $post_data["creator_id"] = $me->id;
        }
        else if($operate_type == 'edit') // 编辑
        {
            $mine = WL_Common_Motorcade::find($operate_id);
            if(!$mine) return response_error([],"该【车队】不存在，刷新页面重试！");
        }
...
//            {
//                $mine_data['superior_motorcade_id'] = $me->motorcade_id;
//            }
            ...
            else throw new Exception("WL_Common_Motorcade--insert--fail");
...
//            exit($e->getMessage());
    ...
    // 【车队】删除
    public function o1__motorcade__item_delete($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'motorcade--item-delete') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        // 判断对象是否合法
        $mine = WL_Common_Motorcade::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【车队】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            if(!$bool) throw new Exception("WL_Common_Motorcade--delete--fail");
            else
            {
...
//            exit($e->getMessage());
    ...
    // 【车队】恢复
    public function o1__motorcade__item_restore($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'motorcade--item-restore') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        try
        {
            $mine->timestamps = false;
            $bool = $mine->restore();
            if(!$bool) throw new Exception("WL_Common_Motorcade--restore--fail");
            else
            {
                $staff_operation_record = new WL_Staff_Record_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    // 【车队】彻底删除
    public function o1__motorcade__item_delete_permanently($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'motorcade--item-delete-permanently') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【车队】管理员-启用
    public function o1__motorcade__item_enable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'motorcade--item-enable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        // 判断对象是否合法
        $mine = WL_Common_Motorcade::find($item_id);
        if(!$mine) return response_error([],"该【车队】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            $mine->timestamps = false;
            $bool = $mine->save();
            if(!$bool) throw new Exception("WL_Common_Motorcade--update--fail");
            else
            {
                $staff_operation_record = new WL_Staff_Record_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    // 【车队】管理员-禁用
    public function o1__motorcade__item_disable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'motorcade--item-disable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【车队】【操作记录】返回-列表-数据
    public function o1__motorcade__item_operation_record_list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;
