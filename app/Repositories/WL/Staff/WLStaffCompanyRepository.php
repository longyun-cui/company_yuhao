...
//            'name.unique' => '该公司号已存在！',
        ...
        $v = Validator::make($post_data, [
            ...
            'name' => 'required',
//            'name' => 'required|unique:WL_Common_Company,name',
        ], $messages);
        if ($v->fails())
        {
            $messages = $v->errors();
        ...
        {
            $is_exist = WL_Common_Company::select('id')->where('name',$post_data["name"])->count();
            if($is_exist) return response_error([],"该【公司】已存在，请勿重复添加！");

            $mine = new WL_Common_Company;
            $post_data["active"] = 1;
            $post_data["creator_id"] = $me->id;
        }
        else if($operate_type == 'edit') // 编辑
        {
            $mine = WL_Common_Company::find($operate_id);
...
//            }
            ...
            else throw new Exception("WL_Common_Company--insert--fail");
...
//            exit($e->getMessage());
    ...
    public function o1__company__item_delete($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = WL_Common_Company::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【公司】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            if(!$bool) throw new Exception("WL_Common_Company--delete--fail");
            else
            {
...
//            exit($e->getMessage());
    ...
    public function o1__company__item_restore($post_data)
    {
        ...
        try
        {
            $mine->timestamps = false;
            $bool = $mine->restore();
            if(!$bool) throw new Exception("WL_Common_Company--restore--fail");
            else
            {
                $staff_operation_record = new WL_Staff_Record_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    public function o1__company__item_enable($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = WL_Common_Company::find($item_id);
        if(!$mine) return response_error([],"该【公司】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            $mine->timestamps = false;
            $bool = $mine->save();
            if(!$bool) throw new Exception("WL_Common_Company--update--fail");
            else
            {
                $staff_operation_record = new WL_Staff_Record_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
