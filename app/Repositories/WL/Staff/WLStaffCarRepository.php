...
//                'latest_order'=>function ($query) { $query->select('id','assign_date','transport_departure_place','transport_destination_place','transport_route'); },
                'latest_order'=>function ($query) { $query->select('*'); },
            ]);

        if(!empty($post_data['id'])) $query->where('id', $post_data['id']);
        if(!empty($post_data['name'])) $query->where('name', 'like', "%{$post_data['name']}%");
        if(!empty($post_data['car_name'])) $query->where('car_name', 'like', "%{$post_data['car_name']}%");
...
//        dd($list->toArray());
    ...
    // 【车辆】近期订单
    public function o1__car__statistic__external_task_recent($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//        dd($order_list->toArray());
        ...
        $order_list = $order_list->groupBy('external_car');
        foreach($order_list as $k => $v)
        {
            $order_list[$k] = $v->keyBy('diff');
        }
