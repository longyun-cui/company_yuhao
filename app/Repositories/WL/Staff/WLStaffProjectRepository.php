...
//        dd($list->toArray());
    ...
    // 【车辆】近期订单
    public function o1__project__statistic__task_recent($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//                "));
        ...
        $order_list = $order_list->groupBy('project_id');
        foreach($order_list as $k => $v)
        {
...
//        dd($order_list);
        ...
        $query = WL_Common_Project::select(['id','item_status','name'])
            ->with([
            ])
            ->where('active',1)
            ->where('item_status',1);
