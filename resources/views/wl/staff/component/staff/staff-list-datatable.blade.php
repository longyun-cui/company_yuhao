<script>
    function Datatable_for_Staff_List($tableId)
    {
...
//                "scrollY": true,
            ...
            "ajax": {
                ...
                "data": function (d) {
                    ...
                    d.staff_type = $tableSearch.find('select[name="staff-staff-type"]').val();
                    d.staff_status = $tableSearch.find('select[name="staff-staff-status"]').val();
                },
            },
            "fixedColumns": {
...
//                            return '<a target="_blank" href="/user/'+data+'">'+row.nickname+'</a>';
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if(data == 1) return '<small class="btn-xs bg-black">BOSS</small>';
                        else if(data == 11) return '<i class="fa fa-gear text-red"></i> 总经理';
                        else if(data == 21) return '<i class="fa fa-diamond text-red"></i> 部门经理';
...
//                            return newDate.toLocaleDateString();
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        {
                            $html_edit = '<a class="btn btn-xs item-admin-edit-submit" data-id="'+data+'">编辑</a>';
                        }
                        ...
                        if(row.user_type == 88)
                        {
                            $html_promote = '<a class="btn btn-xs item-promote-submit" data-id="'+data+'">晋升</a>';
                        }
                        else if(row.user_type == 84)
                        {
                            $html_promote = '<a class="btn btn-xs item-demote-submit" data-id="'+data+'">降职</a>';
                        }
                        else
                        {
                        ...
                        var html =
                            // $html_edit+
                            '<a class="btn btn-xs staff-edit-show" data-id="'+data+'">编辑</a>'+
                            // '<a class="btn btn-xs item-password-admin-change-show" data-id="'+data+'" data-name="'+row.username+'">修改密码</a>'+
                            '<a class="btn btn-xs item-password-reset-submit" data-id="'+data+'">重置密码</a>'+
                            $html_promote+
                            $html_able+
                            $html_delete+
