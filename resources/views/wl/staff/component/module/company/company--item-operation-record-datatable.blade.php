<script>
    var Datatable__for__Company_Item_Operation_Record_List = function ($id) {
        var datatableAjax__company_item_operation_record = function ($id) {

            var $datatable_id = 'datatable--for--company-item-operation-record-list';
            var dt__company_item_operation_record = $('#'+$datatable_id);
            if($.fn.DataTable.isDataTable(dt__company_item_operation_record))
            {
                // 已经初始化
                console.log('#'+$datatable_id+' // 已经初始化');
                $(dt__company_item_operation_record).DataTable().destroy();
            }
            else
            {
                // 未初始化
            ...
            var ajax_datatable__company_item_operation_record = dt__company_item_operation_record.DataTable({
                "retrieve": true,
                "destroy": true,
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "searching": false,
                "ajax": {
                    'url': "/o1/company/item-operation-record-list/datatable-query?id="+$id,
                    "type": 'POST',
                    "dataType" : 'json',
                    "data": function (d) {
                        d._token = $('meta[name="_token"]').attr('content');
                        d.name = $('input[name="company-item-operation-name"]').val();
                        d.title = $('input[name="company-item-operation-title"]').val();
                        d.keyword = $('input[name="company-item-operation-keyword"]').val();
                        d.type = $('select[name="company-item-operation-type"]').val();
                        d.status = $('select[name="company-item-operation-status"]').val();
                    },
                },
...
//                    });
        ...
        return {
            init: datatableAjax__company_item_operation_record
        }
    }();
</script>