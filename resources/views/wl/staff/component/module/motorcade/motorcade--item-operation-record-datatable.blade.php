<script>
    var Datatable__for__Motorcade_Item_Operation_Record_List = function ($id) {
        var datatableAjax__motorcade_item_operation_record = function ($id) {

            var $datatable_id = 'datatable--for--motorcade-item-operation-record-list';
            var dt__motorcade_item_operation_record = $('#'+$datatable_id);
            if($.fn.DataTable.isDataTable(dt__motorcade_item_operation_record))
            {
                // 已经初始化
                console.log('#'+$datatable_id+' // 已经初始化');
                $(dt__motorcade_item_operation_record).DataTable().destroy();
            }
            else
            {
                // 未初始化
            ...
            var ajax_datatable__motorcade_item_operation_record = dt__motorcade_item_operation_record.DataTable({
                "retrieve": true,
                "destroy": true,
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "searching": false,
                "ajax": {
                    'url': "/o1/motorcade/item-operation-record-list/datatable-query?id="+$id,
                    "type": 'POST',
                    "dataType" : 'json',
                    "data": function (d) {
                        d._token = $('meta[name="_token"]').attr('content');
                        d.name = $('input[name="motorcade-item-operation-name"]').val();
                        d.title = $('input[name="motorcade-item-operation-title"]').val();
                        d.keyword = $('input[name="motorcade-item-operation-keyword"]').val();
                        d.type = $('select[name="motorcade-item-operation-type"]').val();
                        d.status = $('select[name="motorcade-item-operation-status"]').val();
                    },
                },
...
//                    });
        ...
        return {
            init: datatableAjax__motorcade_item_operation_record
        }
    }();
</script>