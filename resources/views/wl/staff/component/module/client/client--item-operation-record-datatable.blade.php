<script>
    var Datatable__for__Client_Item_Operation_Record_List = function ($id) {
        var datatableAjax__client_item_operation_record = function ($id) {

            var $datatable_id = 'datatable--for--client-item-operation-record-list';
            var dt__client_item_operation_record = $('#'+$datatable_id);
            if($.fn.DataTable.isDataTable(dt__client_item_operation_record))
            {
                // 已经初始化
                console.log('#'+$datatable_id+' // 已经初始化');
                $(dt__client_item_operation_record).DataTable().destroy();
            }
            else
            {
                // 未初始化
            ...
            var ajax_datatable__client_item_operation_record = dt__client_item_operation_record.DataTable({
                "retrieve": true,
                "destroy": true,
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "searching": false,
                "ajax": {
                    'url': "/o1/client/item-operation-record-list/datatable-query?id="+$id,
                    "type": 'POST',
                    "dataType" : 'json',
                    "data": function (d) {
                        d._token = $('meta[name="_token"]').attr('content');
                        d.name = $('input[name="client-item-operation-name"]').val();
                        d.title = $('input[name="client-item-operation-title"]').val();
                        d.keyword = $('input[name="client-item-operation-keyword"]').val();
                        d.type = $('select[name="client-item-operation-type"]').val();
                        d.status = $('select[name="client-item-operation-status"]').val();
                    },
                },
...
//                    });
        ...
        return {
            init: datatableAjax__client_item_operation_record
        }
    }();
</script>