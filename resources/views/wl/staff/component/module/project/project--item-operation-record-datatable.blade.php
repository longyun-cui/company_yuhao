<script>
    var Datatable__for__Project_Item_Operation_Record_List = function ($id) {
        var datatableAjax__project_item_operation_record = function ($id) {

            var $datatable_id = 'datatable--for--project-item-operation-record-list';
            var dt__project_item_operation_record = $('#'+$datatable_id);
            if($.fn.DataTable.isDataTable(dt__project_item_operation_record))
            {
                // 已经初始化
                console.log('#'+$datatable_id+' // 已经初始化');
                $(dt__project_item_operation_record).DataTable().destroy();
            }
            else
            {
                // 未初始化
            ...
            var ajax_datatable__project_item_operation_record = dt__project_item_operation_record.DataTable({
                "retrieve": true,
                "destroy": true,
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "searching": false,
                "ajax": {
                    'url': "/o1/project/item-operation-record-list/datatable-query?id="+$id,
                    "type": 'POST',
                    "dataType" : 'json',
                    "data": function (d) {
                        d._token = $('meta[name="_token"]').attr('content');
                        d.name = $('input[name="project-item-operation-name"]').val();
                        d.title = $('input[name="project-item-operation-title"]').val();
                        d.keyword = $('input[name="project-item-operation-keyword"]').val();
                        d.type = $('select[name="project-item-operation-type"]').val();
                        d.status = $('select[name="project-item-operation-status"]').val();
                    },
                },
...
//                    });
        ...
        return {
            init: datatableAjax__project_item_operation_record
        }
    }();
</script>