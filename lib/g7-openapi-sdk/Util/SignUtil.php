 ...
 * under the License.
 */
namespace OpenApiSDK\Util;

use OpenApiSDK\Constant\Constants;
use OpenApiSDK\Constant\ContentType;
use OpenApiSDK\Constant\HttpHeader;
use OpenApiSDK\Constant\SystemHeader;
...
class SignUtil
{
    ...
    public static function md5Body($content=''){
	...
	private static function BuildStringToSign($path, $method, &$headers, $querys, $bodys, $signHeaderPrefixList)
    {
		...
		$sb.= Constants::LF;
		if (array_key_exists(HttpHeader::HTTP_HEADER_CONTENT_MD5, $headers) && !array_key_exists(ContentType::CONTENT_TYPE_FORM, $headers)) {
			$sb.= $headers[HttpHeader::HTTP_HEADER_CONTENT_MD5];
		}
		$sb.= Constants::LF;
		if (array_key_exists(HttpHeader::HTTP_HEADER_CONTENT_TYPE, $headers)) {
			$sb.= $headers[HttpHeader::HTTP_HEADER_CONTENT_TYPE];
		}
		$sb.= Constants::LF;
		if (array_key_exists(SystemHeader::X_G7_CA_TIMESTAMP, $headers) && null != $headers[SystemHeader::X_G7_CA_TIMESTAMP]) {
			$sb.= $headers[SystemHeader::X_G7_CA_TIMESTAMP];
		}
		$sb.= Constants::LF;		
    ...
    */
    private static function BuildHeaders(&$headers, $signHeaderPrefixList)
	{
        ...
        unset($signHeaderPrefixList[SystemHeader::X_G7_CA_SIGNATURE]);
        unset($signHeaderPrefixList[SystemHeader::X_G7_CA_TIMESTAMP]);
        ...
        if (is_array($headers)) {
            ...
            foreach ($headers as $itemKey => $itemValue)
            {
                ...
                {
                    if(strlen($itemValue)>0 && strlen($itemKey)>0){
                        $sb .=strtolower($itemKey).Constants::SPE2.$itemValue.Constants::LF;
                    }else{
                        $sb .=strtolower($itemKey).Constants::SPE2.Constants::LF;
                    }
                }
            }
        }
    ...
    */
    private static function IsHeaderToSign($headerName, $signHeaderPrefixList) 
	{
		...
		//@todo
		if (0 === strpos($headerName, Constants::CA_HEADER_TO_SIGN_PREFIX_SYSTEM)) {
			return true;
		}
