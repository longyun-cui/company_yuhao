...
class HttpUtil 
{
	public static function send(HttpRequest $request, $readtimeout, $connectTimeout)
    {
	...
	private static function InitialBasicHeader($path, $appKey, $appSecret, $method, $headers, $querys, $bodys, $signHeaderPrefixList) 
	{
		...
		$headers[SystemHeader::X_G7_CA_TIMESTAMP] = strval(time()*1000);
		//防重放，协议层不能进行重试，否则会报NONCE被使用；如果需要协议层重试，请注释此行
		//$headers[SystemHeader::X_CA_NONCE] = strval(self::NewGuid());

        $sign= SignUtil::Sign($path, $method, $appSecret, $headers, $querys, $bodys, $signHeaderPrefixList);

        $headers[SystemHeader::X_G7_CA_SIGNATURE] ='g7ac '.$appKey.':'.$sign;
